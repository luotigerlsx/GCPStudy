...
def diag_jacobian(xs,
    ...
    with tf.name_scope(name, 'jacobians_diag', [xs, ys]):
        print('Input is:', (xs, ys, sample_shape))
        ...
        print('After convert to tensor', xs)
        if not tfe.executing_eagerly():
            ...
            for y, x in zip(ys, xs):
                ...
                y_ = tf.reshape(y, tf.concat([sample_shape], -1))
                print('Reshape y', y_)
                # Declare an iterator and tensor array loop variables for the gradients.
                n = tf.size(x) / tf.to_int32(tf.reduce_prod(sample_shape))
                print('n', n)
                n = tf.to_int32(n)
                n = tf.cond(tf.equal(n, tf.constant(0, tf.int32)),
                ...
                # zero position
                print('jacobian_diag_res', jacobian_diag_res.stack())
                ...
                print('reshaped_jacobian_diag', reshaped_jacobian_diag)
    ...
    print('final result', jacobians_diag_res)
...
if __name__ == '__main__':
    ...
    f = lambda x: tf.matmul(tf.matmul(Atf, xtf), xtf)

    with tf.Session() as sess:
        # Now let's try to compute the jacobian
        dydx = diag_jacobian(xs=xtf, ys=ytf)
        dydxa = diag_jacobian_pfor(xs=xtf, ys=ytf)
        print(sess.run([dydx, dydxa]))
